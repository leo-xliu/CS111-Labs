...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int exit_status = pthread_mutex_init(&single_lock, NULL);
	if (exit_status != 0){
		exit(exit_status); 
	}
	return hash_table;
                             ...
                             uint32_t value)
{
	...
	int exit_status = pthread_mutex_lock(&single_lock);
	if (exit_status != 0){
		exit(exit_status);
	}
	...
	if (list_entry != NULL) {
		...
		exit_status = pthread_mutex_unlock(&single_lock);
		if (exit_status != 0 ){
		exit(exit_status);
	}
		return;
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	int exit_status = pthread_mutex_destroy(&single_lock);
	if (exit_status != 0){
		exit(exit_status);
	}
	free(hash_table);
