...
void write_superblock(int fd) {
	...
	superblock.s_max_mnt_count     = MAX_MOUNT_COUNT; /* Make this unlimited */
...
void write_block_bitmap(int fd) {
	...
	char *bitmap = calloc(BLOCK_SIZE, sizeof(char));
	if (!bitmap){
		errno_exit("memory allocation");
	}
	int used = LAST_BLOCK;
	for (int i = 0; i < BLOCK_SIZE; i++){ //set all used blocks
		char cur_byte = 0;
		for (int j = 0; j < 8; j++){
			if (used-- > 0){
				cur_byte = cur_byte << 1;
	...
	int needed = NUM_BLOCKS / 8;
	for (int i = needed; i < BLOCK_SIZE; i++){ //set out of range blocks
		bitmap[i] = 0xff;
	}
	needed--; //set bit 1023 to 0 since it represents 1024 
	bitmap[needed] = 0x80;
...
void write_inode_bitmap(int fd) {
	...
	int used = LAST_INO;
	for (int i = 0; i < BLOCK_SIZE; i++){ //set all used inodes
		char cur_byte = 0;
		for (int j = 0; j < 8; j++){
			if (used-- > 0){
				cur_byte = cur_byte << 1;
...
void write_inode_table(int fd) {
	...
	struct ext2_inode ext2_root_inode = {0};
	ext2_root_inode.i_mode = EXT2_S_IFDIR
							| EXT2_S_IRUSR
							| EXT2_S_IWUSR
							| EXT2_S_IXUSR
							| EXT2_S_IRGRP
							...
							| EXT2_S_IROTH
							| EXT2_S_IXOTH;
	ext2_root_inode.i_uid = 0;
	ext2_root_inode.i_size = 1024;
	ext2_root_inode.i_atime = current_time;
	ext2_root_inode.i_ctime = current_time;
	ext2_root_inode.i_mtime = current_time;
	ext2_root_inode.i_dtime = 0;
	ext2_root_inode.i_gid = 0;
	ext2_root_inode.i_links_count = 3;
	ext2_root_inode.i_blocks = 2;
	ext2_root_inode.i_block[0] = ROOT_DIR_BLOCKNO;
	write_inode(fd, EXT2_ROOT_INO, &ext2_root_inode);
	...
	memcpy((char*) hello_inode.i_block, "hello-world", 11);
...
void write_hello_world_file_block(int fd) {
	...
	char* hello_message = calloc(12, sizeof(char));
	if (!hello_message){
		errno_exit("memory allocation");
	}
	memcpy(hello_message, "Hello world\n", 12);

	if (write(fd, hello_message, 12) != 12) {
		errno_exit("write");
	}
